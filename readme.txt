...
build_plus.bat/sh is an alternate build script that uses a better compressor for the sound driver, but the resulting ROM will not be 100% accurate to the original.
...
Non-Windows users will also need to install GCC's C++ compiler ("g++"), as the rest of the build tools are compiled from source when build.sh/build_plus.sh is first ran.
